...
// Additional source code by Tobias Jauch: 17/06/2021 (LSU-v4.1)
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  for (w <- 0 until memWidth) {
    ...
    will_fire_ldq_incoming  (w) := lsu_sched(can_fire_ldq_incoming  (w) , true , false , false , false) // TLB,    ,
    ...
    assert(!(exe_req(w).valid && !(will_fire_load_incoming(w) || will_fire_ldq_incoming(w) || will_fire_stad_incoming(w) || will_fire_sta_incoming(w) || will_fire_std_incoming(w) || will_fire_sfence(w))))
  ...
  for (w <- 0 until memWidth) {
    ...
    when (will_fire_ldq_incoming(w)) //changes made by tojauch for LSU-v4.0
    {
      ...
      assert(!(will_fire_ldq_incoming(w) && ldq_incoming_e(w).bits.addr.valid),
        "[lsu] Incoming load is overwriting a valid address")
    }
